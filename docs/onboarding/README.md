...
- [local-kx.md](./local-kx.md) — land the operator + a smoke-test tenant on the [`kx`](https://github.com/nanohype/kx) kind cluster. Two modes: k8s-only (validates CR emission against real upstream CRDs) + bedrock (mounts laptop AWS creds onto the gateway for end-to-end real Bedrock calls).
