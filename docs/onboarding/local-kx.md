...
| Bedrock `ResourceNotFoundException` on the model | Your account doesn't have access to `us.anthropic.claude-sonnet-4-6` (the route's model). Either request access in the AWS console → Bedrock → Model access, or edit `examples/blank-tenant/platform.yaml`'s `ModelRouteSpec.modelId` to a model you do have.                                                |
