...
| ----------------------------------------------------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tenant A reads Tenant B's S3 prefix                         | **I**nformation Disclosure | Tenant IRSA roles scope `s3:GetObject` to `tenants/<own-platform-id>/*` via path-prefix policy. KMS grant on cmk-data is also scoped per-tenant via `kms:EncryptionContext` matching. |
...
| External attacker steals a leaked tenant IRSA token                     | **S**                | IRSA tokens are short-lived (60-min default, rotated every 30 min) and scoped to the pod's ServiceAccount via projected token volume. Token rotation cannot be turned off.                     |
