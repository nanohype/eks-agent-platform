...
The operator pod assumes the IRSA role provisioned by `landing-zone/components/aws/agent-iam` (`<env>-<cluster>-eks-agent-platform-operator`). The full permission set is the union of policies attached by **three** Terraform components — `agent-iam` owns the role itself plus the IAM/KMS/S3/Bedrock-introspection grants; `cost-access` attaches the Athena/Glue/CloudWatch/CUR policy via `aws_iam_role_policy_attachment.operator_cost` (the pipeline itself is account-scoped in `cost-pipeline`; the grant is per-cluster, so it lives in its per-cluster counterpart); `kill-switch` grants `events:PutEvents` on the breach bus via the bus's resource policy. The "Provisioned in" column below names the component that grants each row.
...
| `kms:CreateGrant`, `RevokeGrant`, `ListGrants`, `DescribeKey`                                                                                                                                                            | platform data key ARN, `kms:GrantIsForAWSResource = true`  | `agent-iam` (`aws_iam_role_policy.operator_kms`)                                                                             | GRANTED BUT UNUSED — the operator makes no KMS call. The grant is residual on the role rather than a capability the reconciler exercises, and removing it from `agent-iam` costs the platform nothing                                                | Operator can grant decrypt access to its tenant roles. **Cannot** decrypt content directly. The `kms:GrantIsForAWSResource = true` condition forbids freeform grants.                                                                                                                                                                                                                                                                                                                                                     |
| `s3:GetBucketPolicy`, `s3:PutBucketPolicy`, `s3:DeleteBucketPolicy`                                                                                                                                                      | model-artifacts bucket ARN only                            | `agent-iam` (`aws_iam_role_policy.operator`, Sid `ArtifactBucketPolicy`)                                                     | Own the model-artifacts bucket policy (extend per-tenant prefixes; delete when the last Platform finalizes) | Operator owns and rewrites the model-artifacts bucket policy. Mitigation: the bucket blocks all public access and objects are SSE-KMS encrypted by default, so the blast radius is per-tenant S3 access, not bucket exposure; policy changes are CloudTrail-logged. The grant is scoped to model-artifacts only — the eval-reports policy is terraform-owned. In-transit TLS is not policy-enforced on this bucket (unlike eval-reports); a baseline `DenyInsecureTransport` seeded by the operator would close that gap. |
...
| `athena:StartQueryExecution`, `GetQueryExecution`, `GetQueryResults`, `StopQueryExecution`, `GetWorkGroup`                                                                                                               | Cost-pipeline Athena workgroup                             | `cost-access` (`aws_iam_role_policy_attachment.operator_cost` → `aws_iam_policy.operator_cost`)                            | Query CUR for per-Platform spend                                                                            | Query-only. No data-modification path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `glue:GetDatabase`, `GetTable`, `GetTables`, `GetPartitions`                                                                                                                                                             | Cost-pipeline catalog only                                 | `cost-access` (same attachment as above)                                                                                   | Read CUR schema                                                                                             | Read-only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `s3:GetObject`, `PutObject`, `ListBucket`                                                                                                                                                                                | Athena results bucket                                      | `cost-access` (same attachment)                                                                                            | Athena output staging                                                                                       | Standard query-result pattern; bucket has 30-day TTL.                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `cloudwatch:GetMetricStatistics`, `GetMetricData`, `ListMetrics`                                                                                                                                                         | `*`                                                        | `cost-access` (same attachment)                                                                                            | Bedrock invocation metrics                                                                                  | Read-only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
3. Grant decrypt access on the platform data key to attacker-controlled tenant roles → read tenant S3 prefixes. Reachable only while the operator role still carries `kms:CreateGrant`; the operator no longer makes that call and the permission's removal from `agent-iam` is the follow-up. **Detection:** `kms:CreateGrant` actions with unfamiliar grantee principals in CloudTrail — and, once the permission is gone, ANY `CreateGrant` by the operator role at all.
...
3. **Revert the operator role** to a fresh provision via `landing-zone/components/aws/agent-iam` (it's stateless apart from the role itself).
...
5. **Rotate the platform data key** if `CreateGrant` actions during the window granted decrypt to unfamiliar principals.
...
| ----------------------------------------------------------- | -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Tenant A reads Tenant B's S3 prefix                         | **I**nformation Disclosure | The operator's per-tenant bucket-policy statement scopes object access to `tenants/<own-platform-id>/*`, and nothing else grants a tenant role access to that bucket, so a cross-tenant read is an implicit deny at S3. This is the ONLY control on that path — KMS cannot discriminate there, because S3's SSE-KMS encryption context is `aws:s3:arn` (the bucket ARN, under a Bucket Key) and every tenant role holds the same `kms:Decrypt` via the baseline. A tenant's OWN datastore key is a separate, resource-scoped grant and does isolate. |
...
| Tenant A exhausts shared Bedrock quota → Tenant B throttled | **D**enial of Service      | Per-route rate limits in `ModelGateway.spec.routes[].rateLimit` enforced by a local `BackendTrafficPolicy` on the gateway. Bedrock account-level quotas remain a shared concern; see [`docs/runbooks/cross-region-fallback.md`](../runbooks/cross-region-fallback.md).        |
...
| Compromised npm package in a workspace package's deps | **T**    | Renovate vulnerability alerts always-on, never auto-merge. OSV alerts enabled. Trivy fs scan in CI fails on HIGH/CRITICAL.                                   |
...
- [x] Data and log keys separable — landing-zone's `secrets` component takes `separate_logs_key`,
      which mints a second CMK and MOVES the CloudWatch Logs and Bedrock grants onto it. Off by
      default: shared is right where the log reader and the data reader are the same person. The
      per-tenant CMK each datastore gets is a separate and stronger control, and holds regardless
- [ ] An auditor principal that can read logs but not data — **not shipped**. landing-zone declares
      an `Auditor` permission set with no account assignments, so it materializes as no IAM role,
      and its `SecurityAudit` policy carries KMS metadata reads and no `kms:Decrypt`. The key
      separation above is the half that now exists; the principal to use it is the half that does not
