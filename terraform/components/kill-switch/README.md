...
Every task retries on backoff and routes to a terminal `RecordFailure` state on exhaustion, so a transient IAM / PutEvents error surfaces in the execution history the `KillSwitchStepFailures` alarm watches rather than dropping silently. Every event is archived to an EventBridge archive with 365-day retention for compliance.
