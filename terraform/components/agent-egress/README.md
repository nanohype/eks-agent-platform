...
- **WAF** (opt-in via `enable_waf`): AWS managed Common + KnownBadInputs rule groups + per-IP rate limit (2000 req/5min). Attached to the model gateway ALB.
...
| `model_gateway_alb_arn`                            | ALB to associate (required when WAF enabled) |
