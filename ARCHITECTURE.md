...
| **Workspace**     | `Platform`                    | `platform`                | —                              | `tenant`         | Tenant `Namespace` (with Pod Security Standards label), `ResourceQuota`, `LimitRange`, default-deny `NetworkPolicy`, ArgoCD `AppProject`, `tenant-runtime` `ServiceAccount`, per-Platform IAM role + Pod Identity association + KMS grant + S3 bucket policy |
