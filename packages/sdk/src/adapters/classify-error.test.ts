...
describe('BedrockAdapter.classifyError', () => {
  it('classifies ThrottlingException as RateLimit', () => {
    expect(adapter.classifyError(new ThrottlingException({ $metadata: {}, message: 'slow down' }))).toBe('RateLimit');
  });

  it('classifies ServiceQuotaExceededException as RateLimit', () => {
    expect(adapter.classifyError(new ServiceQuotaExceededException({ $metadata: {}, message: 'over quota' }))).toBe(
      'RateLimit',
    );
  });

  it('classifies ServiceUnavailableException as Overloaded', () => {
    expect(adapter.classifyError(new ServiceUnavailableException({ $metadata: {}, message: 'busy' }))).toBe(
      'Overloaded',
    );
  });

  it('classifies ValidationException as BadRequest', () => {
    expect(adapter.classifyError(new ValidationException({ $metadata: {}, message: 'bad' }))).toBe('BadRequest');
  });

  it('classifies ResourceNotFoundException as BadRequest', () => {
    expect(adapter.classifyError(new ResourceNotFoundException({ $metadata: {}, message: 'no such model' }))).toBe(
      'BadRequest',
    );
  });

  it('classifies AccessDeniedException as AuthFailure', () => {
    expect(adapter.classifyError(new AccessDeniedException({ $metadata: {}, message: 'denied' }))).toBe('AuthFailure');
  });

  it('classifies InternalServerException as Server', () => {
    expect(adapter.classifyError(new InternalServerException({ $metadata: {}, message: 'oops' }))).toBe('Server');
  });
