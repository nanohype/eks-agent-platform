...
A Kubernetes-native, AWS-native **platform-of-platforms**. Each team's agent workloads are declared as a `Platform` CR; the operator provisions the per-tenant workload namespace, IAM identity, KMS grants, S3 bucket policy, model gateway, agent Deployments, KEDA scaling, budget kill-switch, and Argo-Workflows eval pipeline. A `Tenant` sits above one or more Platforms as the cluster-scoped owner, aggregating their budget and readiness. Nine personas (sales-ops, support, finance, ops, founder, eng, marketing, legal, and the `generic` default) are first-class users with their own onboarding playbooks + agentctl scaffolding.
...
| `operators/` | Go (kubebuilder v4) — one binary, nine reconcilers (`tenant`, `platform`, `modelgateway`, `agentfleet`, `sandboxpool`, `agentsandbox`, `budget`, `slo`, `eval`), one shared leader-election lease. Owns per-tenant AWS state, reconciled from in-cluster under the operator's own IRSA role. Also ships `agentctl` CLI.                                                                                                                                                                                                                                                                                         |
| `charts/`    | Helm — `operator` (CRDs + Deployment + RBAC; ships its own eval-runtime bundle behind `evalRuntime.*`), `bedrock-egress`, `tenant` (opinionated `Platform` CR scaffold).                                                                                                                                                                                                                                                                                                                                                            |
...
| `examples/`  | `blank-tenant` (smoke-test single-agent Platform), `agent-fleet` (two routes, two agents, KEDA SQS scaling).                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
task tofu:apply CLUSTER=development-platform COMPONENT=bedrock
task tofu:apply CLUSTER=org COMPONENT=cost-pipeline
task tofu:apply CLUSTER=development-platform COMPONENT=kill-switch
task tofu:apply CLUSTER=development-platform COMPONENT=eval-runtime
