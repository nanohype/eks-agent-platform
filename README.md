...
| `operators/` | Go (kubebuilder v4) — one binary, nine reconcilers (`tenant`, `platform`, `gateway`, `runtime`, `sandbox`, `agentsandbox`, `budget`, `slo`, `eval`), one shared leader-election lease. Owns per-tenant AWS state, reconciled from in-cluster under the operator's own IRSA role. Also ships `agentctl` CLI.                                                                                                                                                                                                                                                                                         |
